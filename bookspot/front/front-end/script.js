document.addEventListener('DOMContentLoaded', () => {
    ...
    completeSaleButton.addEventListener('click', async () => {
        const metodo_pago = paymentMethodInput.value; // Obtener el método de pago seleccionado

        try {
            // Formatear los productos para que coincidan con la estructura esperada por el backend
            ...
            const response = await fetch('http://127.0.0.1:5000/ventas/completar/', {
                ...
                body: JSON.stringify({ items, metodo_pago }) // Enviar la lista de productos y método de pago
            });

            if (!response.ok) {
                throw new Error(`Error al completar la venta: ${response.statusText}`);
            }

            // Limpiar la lista de productos y re-renderizar la tabla en caso de éxito
            products.length = 0;
            renderProducts();
    ...
    // Cargar los métodos de pago disponibles al cargar la página
    fetchPaymentMethods()
        .then(data => {
            // Llenar el dropdown de métodos de pago
            const paymentMethodSelect = document.getElementById('payment-method');
            data.forEach(method => {
                const option = document.createElement('option');
                option.value = method.nombre;
        ...
        .catch(error => {
            console.error('Error fetching payment methods:', error);
            alert(`Error al obtener métodos de pago: ${error.message}`);
        });
